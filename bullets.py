...
class Big_Bullet:
    def __init__(self, x, y, damage, piercing, exploding, crit):
        ...
        self.ysize = 6
        self.damage = damage * CRITICAL_UPGRADE_DAMAGE_MULTIPLIER if crit else damage
        self.damage = self.damage * BIG_UPGRADE_DAMAGE_MULTIPLIER
        self.piercing = piercing
        self.exploding = exploding
        self.crit = crit
        self.things_hit = []
    ...
    def draw(self):
        pyxel.blt(self.x, self.y, 0, 0, 8, self.xsize, self.ysize, 0)

        # Custom Visuals for different bullet types (stackable)
        if self.exploding: pyxel.rect(self.x, self.y, 2, 3, 8)
        if self.piercing: pyxel.rect(self.x, self.y + 4, 2, 2, 12)
        if self.crit: pyxel.rect(self.x, self.y + 1, 2, 2, 11)